...
7.  Using the data frame you created in Q6 above (`temp`), add a new column to that data frame that converts the temperature column (currently in Celsius) to Fahrenheit. Call the new column `tempF`. 
    ...
    _Hint: we did this in class---use that same equation!_
...
9.  Using the *original* data frame (`weather`), find the minimum (`min()`) temperature recorded for each month (in Celsius, the original column).
    ...
    _Hint: think about months first (split) and then temperature (apply). You will also want to remove all the NA values._
