...
1.  In your own words, describe what the `tidyverse` is. Your answers should be between 1-3 sentences.
...
4.  How many rows and columns are in the data? Type your answers below:

    rows: 139, 144

    columns: 12

5.  Create a data frame that includes temperatures which are above freezer (AKA greater than 0)
...
```{r temp_df}
select(weather, hour:month, temp, station_id)
```
...
9.  Find the minimum temperature recorded for each month (in Celsius). (Hint: think about months first (split) and then temp (apply))
