...
### `slice()` rows out of the data frame

If we want to pull out specific rows based on position alone, we can use the `slice()` function. It works similarly to `select()` for columns except usually uses numbers to indicate positions because rows typically don't have names in R.

```{r slice}
# base R
climate_df[1:4,]

# using slice from the tidyverse
