...
7.  Run the appropriate statistical test, adding species (and the interaction between the two independent variables) into the model. (2 points)
...
#### Question: Is there a difference in the average flipper length between penguin species on Biscoe island?

1.  Our first step is to create a new data frame that includes only the individuals found on Biscoe island. Call this new data frame `biscoe`.
...
We will be using the `biscoe` data frame for the rest of this problem set.

2.  Before we continue, let's change our flipper length column from mm to inches. Each inch has 25.4 mm in it, so we will want to divide our flipper length column by 25.4 to get inches. Name this new column `flipper_length_in`.

    *Hint: make sure you "re-assign" the `biscoe` data frame to itself so the new column shows up in the `biscoe` data frame. After you run the code, you should have 9 variables in `biscoe`.*
...
2.  Now let's summarize our data using this new column. Calculate the mean and standard deviation of the `flipper_length_in` column for each species. (2 points)

```{r}
biscoe %>% 
  group_by(species) %>% 
  summarise(mean_flipper_in = mean(flipper_length_in),
            sd_flipper_in = sd(flipper_length_in))
```
...
```{r}
ggplot(biscoe, aes(species, flipper_length_in)) +
  geom_boxplot() +
  geom_jitter(width = 0.1, alpha = 0.5) +
  labs(x = "Species",
       y = "Flipper Length (in)") +
...
ggplot(biscoe, aes(flipper_length_in, fill = species)) +
  geom_histogram(alpha = 0.5, position = "identity") +
  labs(x = "Flipper Length (in)",
       y = "Frequency",
       fill = "Species") +
  theme_bw()
