...
```{r}
# Show penguin data in environment
penguins <- penguins
```
...
```{r}
# Separate data frame for Dream island
dream <- penguins %>% 
  filter(island == "Dream")
```
...
```{r}
# Mean / SD bill length and mean / SD flipper length
...
```{r}
# Min, max, and mean body mass for each species
body_mass <- penguins %>% 
  group_by(species) %>% 
...
```{r}
# Density plot with line designating the mean
