---
title: 'Exploring `geom` functions'
...
They help us get a feel for the distribution of that data. To make histograms in `ggplot2`, we use the `geom_histogram()` function. Let's look at the number of sick fish.
...
When we create a multiple histogram, we have to add one additional argument to `geom_histogram()`. Let's see what happens if we specify `fill`.
