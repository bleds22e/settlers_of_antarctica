...
2.  Next, we need to load in our tilapia growth dataset using the `read_csv` function. Call the data frame `soy_data`. Then use both the `head` and `tail` functions to take a look at the data. (1 point)
...
```{r}
# if the code isn't running, you probably named the data something different in Question
soy_data <- soy_data %>% 
...
5.  Use the `mutate()` function to add a `day_30_weight_kg` column to the growth data frame. (Hint: divide grams by 1000 to get kg)
    ...
    Save the output as `growth` (2 points)
