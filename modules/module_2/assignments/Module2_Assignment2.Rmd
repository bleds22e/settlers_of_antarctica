...
```{r}
# if the code isn't running, you probably named the data something different in Question 2
growth <- growth %>% 
...
8.  Now we want to use our `tank_category` column in our `group_by` function. We also want `perc_soy_protein` in the group_by function, as well. This will allow us to calculate the average `day_30_weight_kg` for warm and cold tanks in each treatment. Call this new data frame `growth_by_temp_treatment`. (2 points)
