...
-   The dataset now has a `Year` column and a `Month` column that you can use as an independent variable.
...
-   The dataset now has a `Year` and `Month` column in addition to the `Date` column.
...
-   The dataset now has a column for the previous year's December temperature. Because counts of chicks took place in January or February (and nests were counted in October/November), the number of chicks is most likely to be related to the temperatures directly after chicks hatched (December of the previous year).
