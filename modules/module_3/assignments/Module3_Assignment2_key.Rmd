...
```{r}
# Look at first few rows of fish data
head(fish)
```
...
```{r}
# Find average and standard deviation of fish weights in each bay
fish_summary <- fish %>% 
  group_by(bay) %>% 
...
```{r}
# Find average and standard deviation of number of seals in each bay
seal_summary <- seals %>% 
  group_by(bay) %>% 
