...
```{r}
# Mean of wind speed and power output for each turbine company
...
```{r}
# Density plot for power output of each turbine company
...
```{r}
# Box-and-whisker plot of wind speeds between different turbine makers
...
5.  Write a null hypothesis and an alternative hypothesis for the question we are asking and that we will be using statistics to answer. (2 points)
    ...
    **Null Hypothesis** (H~0~): There is no difference in power output and wind speed between the turbine makers.
    **Alternative Hypothesis** (H~A~): There is a difference in power output and wind speed between the turbine makers.
...
```{r}
# t-test of power output by maker
