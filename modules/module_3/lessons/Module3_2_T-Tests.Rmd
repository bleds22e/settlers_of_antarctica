...
In this course so far, we have done the first 2: by calculating the mean values of groups and by plotting histograms and box-plots.
...
We can take a quick look at the data structure to remind ourselves what data we are using.

```{r}

```

Before we dive in, let's talk more about *inferential statistics*. To do so, we'll be using [this powerpoint](https://docs.google.com/presentation/d/1M23a65rkNsF5e3rMrV2OgIv4WVacMyKa/edit?usp=sharing&ouid=103537286572390245832&rtpof=true&sd=true).
...
*(Note: We never "accept" a hypothesis; we "reject" or "fail to reject" the null hypothesis. We don't do this in reference to the alternative hypothesis--only the null hypothesis.)*
...
-   `95 percent confidence interval`: while not exactly accurate, we can think of this as the range that we can expect the difference in the two means to fall in 95% of the time given the data.
    ...
    -   the values are negative because the smaller group (Budget Collars) is first, so we are subtracting the larger number (Collarium average) from the smaller number (Budget Collars)
...
1.  Identify your independent and dependent variables; note whether they are continuous or categorical.
...
-   Is the p-value below our cut-off for significance (0.05)?
-   Is our result statistically significant?
-   Does that mean we should reject or fail to reject the null hypothesis?
-   So...is there a "real" difference in the means between the two companies or not?
