...
-   The variable that we want to compare between groups (dependent or variable) is numerical.
...
#### Does the *battery life* of the collar differ by collar maker?

First, we need to identify our variables. Which variable is the independent variable? What about the dependent variable?

**Null hypothesis** ($H_{0}$):

**Alternative hypothesis** ($H_{A}$):

#### Does the *signal distance* of the collar differ by collar maker?
...
As always, we love visualizing our data, especially as a form of data exploration and comparing means.
...
Let's do a different type of visualization for signal distance. What type of data visualization have we covered that does a good job of showing both central tendency (median, in this case) and variation (middle 50% and 95% confidence intervals) in one numeric variable?
...
Finished these sections already? Take a look around and see if you can help a neighbor.
...
The most common post hoc test for these pairwise comparisons is a Tukey test; we use the `TukeyHSD()` function. It is not the only post hoc test--there are others depending on the specifics of your data set--but it is the only one we will be using. 

You don't need to worry about understanding the nuances of Tukey's test, but you should know how to run the code and interpret it.
...
-   `p adj`: the adjusted p-value for that pair. We interpret it as we interpret any other p-value.
