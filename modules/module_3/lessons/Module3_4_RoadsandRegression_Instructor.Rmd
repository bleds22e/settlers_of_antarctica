...
```{r message=FALSE}
library(tidyverse)
hairgrass_data <- read_csv("../../module_3/data/hairgrass_data.csv")
```
...
To convert the proportion to a percentage, we can multiple the value by 100.
...
This means our equation for the line of best fit is: `y = 1.8x + 84.24` or, with variable names included, `hairgrass density = 1.8*nitrogen + 84.24`. There is a statistically significant relationship between nitrogen content and hairgrass density.
...
Equation for line of best fit, including variable names: hairgrass = -4.648\*soil pH + 150.24
