...
```{r message=FALSE}
library(tidyverse)
hairgrass <- read_csv("../../module_3/data/hairgrass_data.csv")
```
...
```{r}
ggplot(hairgrass, aes(soil_nitrogen, hairgrass_density)) +
  geom_point() +
  labs(x = "N Content",
...
![](images/lines.png){width="48%"} ![](images/residuals.jpg){width="48%"}
...
```{r}
r <- cor(x = hairgrass$soil_nitrogen, y = hairgrass$hairgrass_density)
r
```
...
```{r}
nitrogen <- lm(hairgrass_density ~ soil_nitrogen, hairgrass)
summary(nitrogen)
```
...
-   `b` (y-intercept): part of our line of best fit equation
    -   The "intercept estimate", in this case 82.24, is our y-intercept in our line of best fit
...
This means our equation for the line of best fit is: y = 1.8x + 82.24 and there is a statistically significant relationship between nitrogen content and hairgrass density.
...
```{r}
ggplot(hairgrass, aes(soil_ph, hairgrass_density)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Soil pH",
       y = "Hairgrass Density per m^2") + 
...
```{r}
r <- cor(x = hairgrass$soil_ph, y = hairgrass$hairgrass_density)
```
...
```{r}
soil_ph <- lm(hairgrass_density ~ soil_ph, data = hairgrass)
summary(soil_ph)
```
