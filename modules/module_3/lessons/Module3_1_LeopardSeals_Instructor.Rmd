...
-   calculate the mean and standard deviation for battery life
-   do this for both manufacturers

```{r summarize}
bat_life_summary <- collars %>%
  group_by(maker) %>%
...
in a data frame called `bat_life_summary`. We can now reference that
...
function, we will specifically reference the `bat_life_summary` data
frame.
...
ggplot(collars, aes(battery_life, fill = maker)) +
  geom_histogram(bins = 15, alpha = 0.5, position = "identity") +
  geom_vline(data = bat_life_summary, aes(xintercept = bat_life_mean))
```
...
ggplot(collars, aes(battery_life, fill = maker)) +
  geom_histogram(bins = 15, alpha = 0.5, position = "identity") +
  geom_vline(data = bat_life_summary, aes(xintercept = bat_life_mean, color = maker)) + 
  labs(x = "Battery Life",
       y = "Frequency",
       color = "Maker",
       fill = "Maker") +
...
```{r}
signal_summary <- collars %>%
  group_by(maker) %>%
  summarize(signal_mean = mean(signal_distance),
...
ggplot(collars, aes(signal_distance, fill = maker)) +
  ...
  geom_vline(data = signal_summary, aes(xintercept = signal_mean, color = maker)) + 
...
length is very real when it comes to wildlife tracking! Why do you think
that is?
