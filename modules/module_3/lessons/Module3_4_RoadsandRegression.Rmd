---
title: "Module 3: Roads and Regressions"
...
In your latest assignment, we tried to figure out which bays we should prioritize for fishing based on the following data sets:
...
-   *soil pH*:
-   *nitrogen (N) content*: as percentage per 100 mL soil sample;
-   *phosphorous (P) content*: as percentage per 100 mL soil sample;
-   *percent rock*: how rocky the location is;
-   *max windspeed (mph)*:
-   *average summer temperature (C)*:
-   *penguin density*: the average number of penguins per 5 m\^2 within 100 m of the sample quadrant for hair grass
...
The line of best fit aims to minimize the distance between each observation (points) and the line. The distances between each observation and any line are called *residuals* (the dotted gray lines). The line of best fit is the line that has the smallest *residuals*. ![](images/residuals.jpg) We can add a line of best fit to ggplot by using the `stat_smooth()` function. We need to specify that the method we want should produce a straight line ("linear model").
...
The greater the magnitude (size) of the correlation coefficient, the stronger the relationship between the two variables. ![](images/correlation.jpg)
