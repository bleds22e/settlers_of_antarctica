...
-   The variable that we want to compare between groups (dependent variable) is numeric.

These two aspects above might sound familiar, because they also need to be true for t-tests!

-   When we only have 2 groups to compare, we use a t-test.
...
First up, in small groups, work through the set-up, numeric, and visual sections. We'll spend \~15 minutes working on this.
...
-   Which variable is the independent variable? [collar maker]
-   What about the dependent variable? [battery life]

**Null hypothesis** ($H_{0}$): There is no difference in the average battery life between the three collar makers.\
**Alternative hypothesis** ($H_{A}$): There is a difference in the average battery life the three collar makers.

#### Does the *signal distance* of the collar differ by collar maker?

Let's identify our variables:

-   Independent variable? [collar maker]
-   Dependent variable? [signal distance]
